...
const sunGravConstant = 1.32712440042e20; // gravitational constant for heliocentric orbits
...
export function animate(time) {
  ...
  ORR.sun.rotateOnAxis(
    new THREE.Vector3(0, 1, 0),
    ORR.sun.thetaDot * ORR.times.rate
  );
  let readout = ORR.EphTimeReadout(ORR.times.ephTime).a;
  readout +=
    ORR.times.speed > ORR.times.pauseRate - 4 &&
    ORR.times.speed < ORR.times.pauseRate + 4
      ? ORR.EphTimeReadout(ORR.times.ephTime).b
      : "";
  readout +=
    ORR.times.speed > ORR.times.pauseRate - 2 &&
    ORR.times.speed < ORR.times.pauseRate + 2
      ? ORR.EphTimeReadout(ORR.times.ephTime).c
      : "";
  readout +=
    ORR.times.speed > ORR.times.pauseRate - 4 &&
    ORR.times.speed < ORR.times.pauseRate + 4
      ? ORR.EphTimeReadout(ORR.times.ephTime).d
      : "";
  ...
  if (ORR.state.extraData) {
    document.getElementById("mjd").innerHTML = ORR.EphTimeToMJD(
      ORR.times.ephTime
    ).toFixed(3);
    document.getElementById("lst").innerHTML = ORR.localSiderealTime(
      ORR.times.ephTime
    ).toFixed(3);
  }
  ...
  for (let i = 0; i < ORR.system.length; i++) {
    ...
    body.rotateOnAxis(
      new THREE.Vector3(0, 1, 0),
      ORR.system[i].thetaDot * ORR.times.rate
    );
    ORR.system[i].toSun = ORR.system[i].celestialPos.length();
    ORR.system[i].toEarth = ORR.system[i].celestialPos
      .clone()
      .sub(ORR.system[ORR.specialID.earth].celestialPos);
    ...
    if (redrawLabels) {
      if (tag != undefined) {
        if (
          Math.abs(tagPos.x - ORR.center.x) < ORR.center.x &&
          Math.abs(tagPos.y - ORR.center.y) < ORR.center.y &&
          tagPos.z < 1
        ) {
  ...
  for (let i = 0; i < ORR.moons.length; i++) {
    ORR.paths[ORR.moons[i].path].position.x =
      ORR.system[ORR.paths[ORR.moons[i].path].orbitId].celestialPos.x;
    ORR.paths[ORR.moons[i].path].position.y =
      ORR.system[ORR.paths[ORR.moons[i].path].orbitId].celestialPos.y;
    ORR.paths[ORR.moons[i].path].position.z =
      ORR.system[ORR.paths[ORR.moons[i].path].orbitId].celestialPos.z;
  }
  ORR.system[ORR.specialID.earth].baryPos = ORR.system[
    ORR.specialID.earth
  ].celestialPos
    .clone()
    .sub(ORR.system[ORR.specialID.moon].celestialPos)
    .multiplyScalar(earthBary);
  ...
  const plutoBaryOffset = ORR.system[ORR.specialID.pluto].celestialPos
    .clone()
    .sub(ORR.system[ORR.specialID.charon].celestialPos)
    .multiplyScalar(plutoBary);
  ...
  if (ORR.state.clickedLabel != "") {
    ...
    const elongation =
      180 -
      ORR.state.clickedPlanet.toEarth.angleTo(
        ORR.system[ORR.specialID.earth].celestialPos
      ) *
        ORR.toDeg;
    if (liveData) {
      if (typeof ORR.state.clickedPlanet.orbitId == "undefined") {
        document.getElementById("orbitVel").innerHTML = ORR.visViva(
          sunGravConstant,
          ORR.state.clickedPlanet.toSun,
          ORR.state.clickedPlanet.semiMajorAxis
        ).toFixed(3);
      } else {
        const toOrbiting = ORR.state.clickedPlanet.celestialPos
          .clone()
          .sub(ORR.system[ORR.state.clickedPlanet.orbitId].celestialPos)
          .length();
        document.getElementById("orbitVel").innerHTML = ORR.visViva(
          ORR.system[ORR.state.clickedPlanet.orbitId].mass * gravConstant,
          toOrbiting,
          ORR.state.clickedPlanet.semiMajorAxis
        ).toFixed(3);
      }
      ...
      if (AltAz.alt > 0) {
        ...
        magNote =
          "<br>(" +
          adjMag.mag.toFixed(2) +
          " under " +
          adjMag.airmass.toFixed(2) +
          " airmasses)";
      }
      document.getElementById("appMag").innerHTML = appMag.toFixed(2) + magNote;
      document.getElementById("toSun").innerHTML =
        ORR.state.clickedPlanet.toSun.toFixed(4);
      document.getElementById("earthToSun").innerHTML =
        ORR.state.clickedPlanet.toSun.toFixed(4);
      document.getElementById("toEarth").innerHTML =
        ORR.state.clickedPlanet.toEarth.length().toFixed(4);
      ...
      if (ORR.state.extraData) {
        ...
        document.getElementById("ha").innerHTML =
          haDMS.deg + "h " + haDMS.min + "&rsquo; " + haDMS.sec.toFixed(1);
      }
      document.getElementById("elong").innerHTML = elongation.toFixed(3);
      if (ORR.state.hoverLabel) {
        const toActive = ORR.state.clickedPlanet.celestialPos
          .clone()
          .sub(ORR.system[ORR.state.hoverLabel[0].id].celestialPos)
          .length();
        const activeOut =
          toActive < 0.001
            ? (toActive * ORR.AU).toFixed(1) + " km"
            : toActive.toFixed(3) + " AU";
  ...
  if (TWEEN.getAll().length == 0) {
    // lock on target after the tween
    ORR.controls.target =
      ORR.state.clickedLabel == ""
        ? new THREE.Vector3()
        : ORR.state.clickedPlanet.celestialPos;
