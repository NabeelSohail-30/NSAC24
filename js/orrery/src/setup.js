...
export const AU = 1.495978707e11; // astronomical unit
...
export const rates = [
  ...
  -20 / daysPerCent / fps,
  -1 / daysPerCent / fps,
  -1 / 24 / daysPerCent / fps,
  -1 / 86400 / daysPerCent / fps,
  0,
  1 / 86400 / daysPerCent / fps,
  1 / 24 / daysPerCent / fps,
  1 / daysPerCent / fps,
...
export const cameraLocked = {
  starfieldObj: new THREE.Object3D(),
  graticule: new THREE.Line(),
};
...
if (
  typeof vars.t != "undefined" &&
  (vars.t.length == 12 || vars.t.length == 13)
) {
  // apply date
...
export const defaultMaterial = new THREE.MeshStandardMaterial({
  map: loader.load("data/1k_eris_fictional.jpg"),
});
...
export const transparentMaterial = new THREE.LineBasicMaterial({
  transparent: true,
  opacity: 0,
});
...
const sunMaterial = new THREE.MeshBasicMaterial({
  map: loader.load("data/1k_sun.jpg"),
});
...
const camStart = new THREE.Vector3(0, -8, 0).applyAxisAngle(
  celestialXAxis,
  eclInclination
);
