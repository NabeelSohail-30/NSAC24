 ...
 */
export function makeBody(sysId, radius, ringRad) {
  // make bodies
  const body = ORR.system[sysId];
  const material =
    body.texture != "default"
      ? new THREE.MeshStandardMaterial({
          map: ORR.loader.load("data/" + body.texture),
        })
      : ORR.defaultMaterial;
  ...
  sphere.rotateOnAxis(
    ...
    body.thetaDot * ORR.times.ephTime + Math.PI * body.phase
  );
  return sphere;
}
...
export function makeLabel(i) {
  $("body").append(
    "<div id='" + i + "' class='label'>" + ORR.system[i].label + "</div>"
  );
  $("#" + i)
    .addClass("tag" + ORR.system[i].type)
    .on("click", function () {
      $(".label").removeClass("active");
      if (
        ORR.state.clickedLabel != "" &&
        $(this)[0].id == ORR.state.clickedLabel[0].id
      ) {
    ...
    .hover(
      function () {
        if (
          ...
          ORR.state.hoverLabel.html(
            hoverContent + '<span id="distToActive"></span>'
          );
        }
      },
      function () {
...
export function makeGratLabel(i, text) {
  $("body").append(
    "<div id='grat" + i + "' class='gratLabel'>" + text + "</div>"
  );
  return "grat" + i;
}
...
export function makeRefPoints() {
  ...
  for (let i = 1; i < latDivisions; i++) {
    ...
    for (let j = 0; j < longDivisions; j++) {
      const x =
        Math.sin((j * Math.PI * 2) / longDivisions) *
        Math.sin((i * Math.PI) / latDivisions) *
        gratRadius;
      const y = Math.cos((i * Math.PI) / latDivisions) * gratRadius;
      const z =
        Math.cos((j * Math.PI * 2) / longDivisions) *
        Math.sin((i * Math.PI) / latDivisions) *
        gratRadius;
