...
export function plotPoint(meanAnomaly, eccentricity, semiMajorAxis, runKepler) {
  const eccAnomaly =
    runKepler && ORR.orbitPlot.points == 1
      ? kepler(eccentricity, meanAnomaly)
      : meanAnomaly;
  const localPoint = new THREE.Vector2(
    semiMajorAxis * (Math.cos(eccAnomaly) - eccentricity),
    semiMajorAxis *
 ...
 */
export function kepler(e, m) {
  // numerical approximation of Kepler's equation
  let result = 0;
 ...
 */
export function hypKepler(e, m) {
  // numerical approximation of hyperbolic Kepler's equation
  let result = 0;
  let lastResult, delta;
  const tolerance = 0.00000001;
  do {
 ...
 */
export function celestial_THREE(
  longPeriapsis,
  longAscNode,
  inclination,
  xLocal,
  ...
  v.applyAxisAngle(ORR.celestialZAxis, longPeriapsis)
    .applyAxisAngle(ORR.celestialXAxis, inclination)
    .applyAxisAngle(ORR.celestialZAxis, longAscNode);
...
export function planetary(
  ...
  let v = new THREE.Vector3(xLocal, 0, yLocal).applyAxisAngle(
    ORR.celestialZAxis,
    longPeriapsis + longAscNode
  );
  const y = new THREE.Vector3(0, 0, 1);
  switch (orbitRef) {
    case "L": // relative to local Lagrangian
      v.applyAxisAngle(y, Math.PI / 2 - dec).applyAxisAngle(
        ORR.celestialZAxis,
        ra
      );
      break;
    case "Q": // relative to the planet's equator
      v.applyAxisAngle(
        y,
        Math.PI / 2 - ORR.system[orbitId].axisDec + inclination
      ).applyAxisAngle(ORR.celestialZAxis, ORR.system[orbitId].axisRA);
      break;
...
export function celestial(
  ...
  const cosW = Math.cos(longPeriapsis);
  const sinW = Math.sin(longPeriapsis);
  const cosO = Math.cos(longAscNode);
  const sinO = Math.sin(longAscNode);
  const cosI = Math.cos(inclination);
  const sinI = Math.sin(inclination);
  const x =
    (cosW * cosO - sinW * sinO * cosI) * xLocal +
    (-1 * sinW * cosO - cosW * sinO * cosI) * yLocal;
  const y =
    (cosW * sinO + sinW * cosO * cosI) * xLocal +
    (-1 * sinW * sinO + cosW * cosO * cosI) * yLocal;
  const z = sinW * sinI * xLocal + cosW * sinI * yLocal;
  return new THREE.Vector3(x, z, y).applyAxisAngle(
...
export function orbitPath(i) {
  ...
  const path = new THREE.LineLoop(
    orbitGeometry,
    ORR.pathMaterials[ORR.system[i].type]
  );
...
export function redraw(i) {
  ...
  ORR.system[i].updateOrbit();
  ORR.paths[i].geometry = new THREE.BufferGeometry().setFromPoints(
    ORR.system[i].celestial
  );
...
export function RADecToVector(ra, dec) {
  const v = new THREE.Vector3(0, 1, 0).applyAxisAngle(
    new THREE.Vector3(1, 0, 0),
    (90 - dec) * ORR.toRad
  );
  return v.applyAxisAngle(
    new THREE.Vector3(0, 1, 0),
    ((ra + 6) % 24) * 15 * ORR.toRad
  );
}
...
export function vectortoRadec(v) {
  return {
    ra: ((Math.atan2(v.x, v.z) * ORR.toDeg) / 15 + 42) % 24,
 ...
 * @returns {object}
 */
export function EphTimeReadout(d) {
  // display time
  const t = new Date(
    (d * ORR.daysPerCent + J2KInMJD - UnixTimeZeroInMJD) * DayInMillis
  );
  ...
  return {
    a:
      ...
      (" " + t.getDate()).slice(-2) +
      ", " +
      Math.abs(parseInt(t.getFullYear())).toString() +
      era,
    b:
      "&nbsp;&nbsp;&bull;&nbsp;&nbsp;" +
      (" " + b).slice(-2) +
...
export function localSiderealTime(ephTime) {
  ...
  const timeUTC =
    ...
    t.getUTCMilliseconds() / 3600000;
  return (
    (100.46 +
      0.985647 * ephTime * ORR.daysPerCent +
      ORR.groundPosition.longitude +
      15 * timeUTC +
      360) %
    360
  );
...
export function getRA(obj) {
  ...
  const earthSurfPos = new THREE.Vector3(
    earthRad *
      Math.cos(
        ORR.groundPosition.longitude * ORR.toRad +
          ORR.system[ORR.specialID.earth].phase
      ),
    earthRad * Math.sin(ORR.groundPosition.latitude * ORR.toRad),
    earthRad *
      Math.sin(
        ORR.groundPosition.longitude * ORR.toRad +
          ORR.system[ORR.specialID.earth].phase
  ...
  const parallaxPos = ORR.system[ORR.specialID.earth].celestialPos
    .clone()
    .add(earthSurfPos)
    .add(ORR.system[ORR.specialID.earth].baryPos);
  return vectortoRadec(
    obj.sysId == ORR.specialID.earth
      ? parallaxPos.multiplyScalar(-1)
      : obj.celestialPos.clone().sub(parallaxPos)
  );
}
...
export function riseSet(obj) {
  ...
  for (let i = 0; i < 1441; i++) {
    ...
    if (altA * altB < 0) {
      crossings.push({
        type: altA > 0 ? "Sets: " : "Rises: ",
        time:
          EphTimeReadout(startTime + i * min).b.substr(30) +
          EphTimeReadout(startTime + i * min).d,
      });
    }
  }
...
export function dateCodeToMJD(d) {
  ...
  const date =
    ...
    d.substr(6, 2) +
    "T" +
    ("0" + dayPart.deg.toString().slice(-1)) +
    ":" +
    ("0" + dayPart.min.toString().slice(-1)) +
    ":" +
    ("0" + dayPart.sec.toString().slice(-1));
...
export function apparentMag(i) {
  ...
  return (
    i.absoluteMag +
    5 * Math.log10(dBS * dBO) -
    2.5 * Math.log10(i.phaseIntegral(alpha))
  );
}
...
export function extinction(magnitude, alt) {
  ...
  const airmass = Math.min(
    1 / Math.cos(angle * ORR.toRad),
    Math.max(20, 6.2 * angle - 520)
  );
