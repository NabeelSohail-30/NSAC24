...
function fullLoad() {
  ...
  //         const vector = ORR.RADecToVector(
  //           parseFloat(starData[i].ra),
  //           parseFloat(starData[i].dec)
  //         ).multiplyScalar(900);
  //         positions.push(vector.x, vector.y, vector.z);
  //         const luma = Math.pow(10 / (parseFloat(starData[i].mag) + 10), 4);
  //         const cindex =
  //           typeof starData[i].ci != "undefined" && starData[i].ci.length > 0
  //             ? parseFloat(starData[i].ci)
  //             : 0;
...
function launch() {
  ...
  if (flags == datasets) {
    ...
    for (let i = 0; i < ORR.moons.length; i++) {
      ORR.moons[i].orbitId = ORR.searchLists.orderedNames.findIndex(
        (e) => e == ORR.moons[i].orbiting
      );
    }
    for (let i = 0; i < ORR.system.length; i++) {
      ...
      if (ORR.system[i].type < 3 || ORR.system[i] instanceof ORR.Moon == true) {
        ORR.scene.add(path);
        ORR.scene.add(
          ORR.makeBody(i, ORR.system[i].exagRadius, ORR.system[i].ringRadius)
        );
        ORR.makeLabel(i);
      } else {
    ...
    for (let i = 0; i < ORR.moons.length; i++) {
      ...
      if (rad > ORR.system[ORR.moons[i].orbitId].largestMoonRadius) {
        ORR.system[ORR.moons[i].orbitId].secondMoon =
          ORR.system[ORR.moons[i].orbitId].largestMoon;
    ...
    // barycentric bodies
    ORR.specialID.earth = ORR.searchLists.orderedNames.findIndex(
      (e) => e == "Earth"
    );
    ORR.specialID.moon = ORR.searchLists.orderedNames.findIndex(
      (e) => e == "Moon"
    );
    ORR.specialID.pluto = ORR.searchLists.orderedNames.findIndex(
      (e) => e == "Pluto"
    );
    ORR.specialID.charon = ORR.searchLists.orderedNames.findIndex(
      (e) => e == "Charon"
    );

    for (let i = 0; i < ORR.precessing.length; i++) {
      ORR.precessing[i] = ORR.searchLists.orderedNames.findIndex(
        (e) => e == ORR.precessing[i]
      );
    }
    ...
    ORR.searchLists.combined = ORR.searchLists.planetNames.concat(
      ORR.searchLists.moonNames,
      ORR.searchLists.asteroidNames,
      ORR.searchLists.cometNames
    );
